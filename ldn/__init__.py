...
class APNetwork:
            ...
            while True:
                ...
                if isinstance(event, wlan.CustomFrameEvent):
                    ...
                    if new_participant is not None:
                        mac = new_participant.mac_address
                        if mac not in self._arp_channels:
                            send_ch, recv_ch = trio.open_memory_channel(8)
                            self._udp_channels[mac] = send_ch
                            self._arp_channels[mac] = send_ch
    ...
    async def _send_qos_null(self, participant: ParticipantInfo) -> None:
        frame = wlan.DataFrame()
        frame.subtype = 12
        frame.target = participant.mac_address
        frame.source = self._interface.address()
    ...
    async def _do_udp_handshake(
        ...
        try:
            ...
            # Register port 5002 once — kept alive across all sequential connections for this participant
            ...
            out_conn_idx = 0
            try:
                while True:  # one iteration per TCP connection pair
                    tcp_src_port = 49200 + out_conn_idx
                    ...
                    try:
                        # Outgoing TCP handshake: our tcp_src_port → their 5002
                        logger.info("Sending TCP SYN to %s (port %d)", participant.mac_address, tcp_src_port)
                        ...
                        # Incoming TCP handshake: their 49*** → our 5002
                        ...
                        async def _outgoing_ack_task() -> None:
                            ...
                            while True:
                                ...
                                if isinstance(signal, tuple) and signal[0] == 'ack':
                                    ...
                                    ip_id = (ip_id + 1) & 0xFFFF
                                    await self._send_tcp(participant, tcp_src_port, server_port, b'',
                                                         seq=our_seq_data, ack_num=new_ack, flags=0x11,
                                                         ip_id=ip_id)
                                    ip_id = (ip_id + 1) & 0xFFFF
                        ...
                        async with trio.open_nursery() as _kn:
                            ...
                            while True:
                                ...
                                if isinstance(signal, tuple) and signal[0] == 'data':
                                    ...
                                    ip_id = (ip_id + 1) & 0xFFFF
                                    await self._send_tcp(participant, 5002, client_src_port, b'',
                                                         seq=in_our_seq, ack_num=new_ack, flags=0x11,
                                                         ip_id=ip_id)
                                    ip_id = (ip_id + 1) & 0xFFFF
                        ...
                        # Connection pair complete — advance to next port index and fresh ISN
                        out_conn_idx += 1
                        in_isn = random.randint(0, 0xFFFFFFFF)
                    finally:
    ...
    def _check_udp_handshake(self, source: MACAddress, data: bytes) -> bool:
        ...
        if protocol == 1:  # ICMP — Destination Unreachable triggers a retry
            ...
            if (src_port == 5001 and dst_port == 5001
                    and payload.startswith(b'\x01\x00\x00\x00')):
                try:
    ...
    def _check_tcp_handshake(self, source: MACAddress, data: bytes) -> bool:
        """Returns True if the packet is a TCP handshake frame and should be consumed."""
        if source not in self._tcp_channels:
            return False
        if len(data) < 20:
            return False
        ihl = (data[0] & 0x0F) * 4
        ...
        tcp_hlen = (tcp[12] >> 4) * 4
        ip_total = struct.unpack_from("!H", data, 2)[0]
    ...
    async def _process_disassociation(self, address: MACAddress) -> None:
        ...
        self._network.application_data = self._param.application_data
        self._seq_num = 1
        self._update_nonce()
    ...
    async def _receive_data_frames(self) -> None:
        while True:
            ...
            if isinstance(frame, wlan.DataFrame):
                ...
                except (ValueError, struct.error, streams.ParseError):
    ...
    async def _process_data_frame(self, frame: wlan.DataFrame) -> None:
        ...
        if snap.protocol == wlan.ETH_P_IP:
            ...
            if self._check_udp_handshake(frame.source, snap.payload):
                return
            if self._check_tcp_handshake(frame.source, snap.payload):
                return
